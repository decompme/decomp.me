...
from .models.flags import Flags, Language
...
DUMMY = DummyCompiler(id="dummy", platform=DUMMY_PLATFORM, cc="")

DUMMY_LONGRUNNING = DummyLongRunningCompiler(
    id="dummy_longrunning", platform=DUMMY_PLATFORM, cc="sleep 3600"
)
...
_compilers = dict({c.id: c for c in _all_compilers if c.available()})

# TODO: logging is not configured at this point, so these go nowhere...
