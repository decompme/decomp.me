...
PSYQ_MSDOS_CC = (
    ...
    '(HOME="$(pwd)" /usr/bin/dosemu -quiet -dumb -f .dosemurc -no-priv-sep -K "${COMPILER_DIR}" -E "D:\\COMPILE.BAT") && '
    '(HOME="$(pwd)" /usr/bin/dosemu -quiet -dumb -f .dosemurc -no-priv-sep -K "${COMPILER_DIR}" -E "ASPSX.EXE -quiet D:\\output.s -o D:\\output.obj") && '
...
SATURN_CC = (
    ...
    '(HOME="$(pwd)" /usr/bin/dosemu -quiet -dumb -f .dosemurc -no-priv-sep -K "${COMPILER_DIR}" -E "CPP.EXE D:\\dos_src.c -o D:\\src_proc.c") && '
    '(HOME="$(pwd)" /usr/bin/dosemu -quiet -dumb -f .dosemurc -no-priv-sep -K "${COMPILER_DIR}" -E "CC1.EXE -quiet ${COMPILER_FLAGS} D:\\src_proc.c -o D:\\output.s") && '
    '(HOME="$(pwd)" /usr/bin/dosemu -quiet -dumb -f .dosemurc -no-priv-sep -K "${COMPILER_DIR}" -E "AS.EXE D:\\output.s -o D:\\output.o") && '
...
BORLAND_MSDOS_CC = (
    ...
    'cat "${INPUT}" | unix2dos > dos_src.c && '
    '(HOME="$(pwd)" /usr/bin/dosemu -quiet -dumb -f .dosemurc -no-priv-sep -K . -E "D:\\bin\\bcc.exe -ID:\\include ${COMPILER_FLAGS} -c -oout.o dos_src.c") && '
    'cp out.o "${OUTPUT}"'
)
