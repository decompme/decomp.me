...
class PresetSerializer(serializers.ModelSerializer[Preset]):
    ...
    def validate_platform(self, platform_id: str) -> str:
        ...
        try:
            return cromper.get_platform_by_id(platform_id).id
        except Exception:
    ...
    def validate_compiler(self, compiler_id: str) -> str:
        cromper = get_cromper_client()
        try:
            return cromper.get_compiler_by_id(compiler_id).id
        except Exception:
...
class ScratchCreateSerializer(serializers.Serializer[None]):
    ...
    libraries = serializers.JSONField(default=list)  # type: ignore[arg-type,assignment]
...
class ScratchCompileSerializer(serializers.Serializer[None]):
    ...
    def validate(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        compiler = get_cromper_client().get_compiler_by_id(compiler_id)
        if compiler.platform.id != scratch.platform:
            raise serializers.ValidationError(
                f"Compiler {compiler.id} is not compatible with platform {scratch.platform}"
            )
        return data
...
class ScratchSerializer(serializers.ModelSerializer[Scratch]):
    ...
    def validate(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if compiler.platform.id != platform_id:
            raise serializers.ValidationError(
                f"Compiler {compiler.id} is not compatible with platform {platform_id}"
            )
        return data
