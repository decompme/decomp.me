...
const useLast = <T>(values: readonly T[]) =>
    values.reduce((_, v) => v, "" as T);
...
class LanguageServerPlugin implements PluginValue {
    ...
    async save(contents: string) {
        this.client.transportWriteFile(
            this.documentUri.split("file://")[1],
            contents,
        );
...
function languageServerWithTransport(
    options: LanguageServerOptions,
): [Extension[], (contents: string) => Promise<void>] {
...
function formatContentEntry(
    entry: LSP.MarkupContent | LSP.MarkedString,
): string[] {
